...
export default function TabDispensadores({ cliente, onClienteUpdate, showSuccess, showError }: Props) {
  ...
  // Comprimir imágenes para que no desborden el tamaño máximo de Server Actions
  const redimensionarYComprimir = (imgSrc: string, callback: (b64: string) => void) => {
    const img = new Image();
    img.src = imgSrc;
    img.onload = () => {
      const maxDim = 800; // Máximo 800px para mantenerlo muy liviano
      let width = img.width;
  ...
  const startCamera = async () => {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      showError('Cámara no disponible', 'Tu navegador o entorno (requiere HTTPS o localhost) no permite acceso a la cámara.');
      return;
    }
  ...
  // Función para descartar la fotografía
