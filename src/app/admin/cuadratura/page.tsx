...
import { TipoTransaccion, TipoCliente, MetodoPago, Prisma } from '@/src/lib/prisma/generated';
...
export async function obtenerCuadraturasAction(desde?: string, hasta?: string, usuario_id?: string) {
  try {
    const where: Prisma.CuadraturaWhereInput = {};
    ...
    if (desde || hasta) {
      ...
      if (desde) {
        ...
        if (!isNaN(fechaDesde.getTime())) where.fecha.gte = fechaDesde;
      }
      if (hasta) {
        const fechaHasta = new Date(`${hasta}T23:59:59`);
        if (!isNaN(fechaHasta.getTime())) where.fecha.lte = fechaHasta;
      }
    }
    ...
    return { success: false, cuadraturas: [], message: error instanceof Error ? error.message : 'Error desconocido' };
  }
}
...
export async function obtenerRepartidoresCuadraturaAction() {
  try {
    ...
    return { success: false, repartidores: [], message: error instanceof Error ? error.message : 'Error desconocido' };
  }
}
...
export async function obtenerProductosCuadraturaAction() {
  try {
    ...
    return { success: false, productos: [], message: error instanceof Error ? error.message : 'Error desconocido' };
  }
}
...
export async function obtenerStockAction() {
  try {
    ...
    return { success: false, stockFabrica: [], stockCamion: [], message: error instanceof Error ? error.message : 'Error desconocido' };
  }
}
...
export async function obtenerGuiasRepartidorDiaAction(usuario_id: string, fecha: string) {
  try {
    ...
    if (isNaN(inicio.getTime()) || isNaN(fin.getTime())) return { success: true, guias: [] };
    ...
    return { success: false, guias: [], message: error instanceof Error ? error.message : 'Error desconocido' };
  }
}
...
export async function registrarSalidaAction(data: SalidaInput) {
  try {
    ...
    const cuadraturaId = await prisma.$transaction(async (tx) => {
      ...
      if (cuadratura) {
        // Mantenemos ejecución secuencial para evitar bloqueos en la BD
      ...
      // Precargamos la información de productos una sola vez
      const productosIds = items.map(i => i.producto_id);
      const productosData = await tx.producto.findMany({ where: { id: { in: productosIds } } });
    ...
    return { success: false, message: error instanceof Error ? error.message : 'No se pudo registrar la salida.' };
  }
}
...
export async function registrarCierreCuadraturaAction(data: CierreCuadraturaInput) {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // Reversión secuencial para no sobreescribir updates concurrentes
      ...
      // Optimización N+1: Obtenemos todos los productos y comisiones asociados ANTES de iterar
      const itemsVentasValidas = data.ventas.filter((it) => it.cantidad > 0);
      const productosIdsVentas = [...new Set(itemsVentasValidas.map(v => v.producto_id))];
      
      const productosData = await tx.producto.findMany({ where: { id: { in: productosIdsVentas } } });
      const productosMap = new Map(productosData.map(p => [p.id, p]));
      
      let comisionesData: any[] = [];
      if (usuario.rol === 'REPARTIDOR' && usuario.recibe_comision) {
        comisionesData = await tx.comision.findMany({ where: { producto_id: { in: productosIdsVentas } } });
      }

      for (const v of itemsVentasValidas) {
        const producto = productosMap.get(v.producto_id);
        if (!producto) continue;

        let comision = 0;
        if (usuario.rol === 'REPARTIDOR' && usuario.recibe_comision) {
          // Búsqueda en memoria, costo DB: 0
          const comisionCfg = comisionesData.find(c => 
            c.producto_id === v.producto_id && 
            c.tipo_transaccion === v.tipo_transaccion && 
            c.tipo_cliente === v.tipo_cliente
          );
      ...
      const stocksCamionFinal = await tx.stockCamion.findMany({ 
        where: { usuario_id: cuadratura.usuario_id },
        include: { producto: { select: { nombre: true } } }
      });

      for (const sc of stocksCamionFinal) {
        if (sc.cantidad < 0) {
          alertas.push(`Stock en camión de ${sc.producto?.nombre || 'un producto'} quedó negativo (${sc.cantidad}). Revisa las cantidades ingresadas.`);
        }
      }
    });
    ...
    return { success: false, message: error instanceof Error ? error.message : 'No se pudo registrar el cierre de la cuadratura.' };
  }
}
...
export async function reabrirCuadraturaAction(cuadraturaId: string, motivo: string) {
  try {
    ...
    return { success: false, message: error instanceof Error ? error.message : 'No se pudo reabrir la cuadratura.' };
  }
}